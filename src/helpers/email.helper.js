...
const transporter = nodemailer.createTransport({
  host: 'smtp.ethereal.email',
    port: 587,
    auth: {
        user: 'alana.leffler21@ethereal.email',
        pass: '9Eveqbf2nqJJuG57yg'
    }
});
...
const emailProcessor = ({ email, pin, type, verificationLink = "" }) => {
  ...
  switch (type) {
    ...
    case "update-password-success":
      info = {
        from: '"Edvora Company" <abe.kohler59@ethereal.email>', // sender address
        to: email, // list of receivers
    ...
    case "new-user-confirmation-required":
      info = {
        from: '"CMR Company" <alana.leffler21@ethereal.email>', // sender address
        to: email, // list of receivers
